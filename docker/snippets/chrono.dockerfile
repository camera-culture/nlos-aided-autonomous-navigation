...
# Blaze
RUN git clone https://bitbucket.org/blaze-lib/blaze.git /opt/blaze && \
    mkdir -p /opt/blaze/build && \
    cd /opt/blaze/build && \
    cmake .. -DCMAKE_INSTALL_PREFIX=/opt/blaze/install && \
    make && make install
...
RUN . ${ROS_WORKSPACE_DIR}/install/setup.sh && \
    ...
    cmake ../ -G Ninja \
        ...
        -DBLAZE_INSTALL_DIR=/opt/blaze/install/include \
